...
def lancementTirage(tirages, labels, etat, T, label_nb):
    if etat.valeur == 0:
        ...
        x = [100, 170, 240, 310, 380, 450, 520, 590, 660, 730]
        y = [70, 120, 190, 260]
        namefont = "Helvetica"
        fontsize = 50
        etat.tirage = tirage
        ...
        label_nb.configure(text='tirage '+str(etat.nb))
        ...
        if len(tirage) == 10:
            ...
            labels[1].place(x=x[1], y=y[0], anchor=CENTER)
            labels[1].configure(font=(namefont, fontsize), text=tirage[1])

            labels[2].place(x=x[2], y=y[0], anchor=CENTER)
            labels[2].configure(font=(namefont, fontsize), text=tirage[2])

            labels[3].place(x=x[3], y=y[0], anchor=CENTER)
            labels[3].configure(font=(namefont, fontsize), text=tirage[3])

            labels[4].place(x=x[4], y=y[0], anchor=CENTER)
            labels[4].configure(font=(namefont, fontsize), text=tirage[4])

            labels[5].place(x=x[5], y=y[0], anchor=CENTER)
            labels[5].configure(font=(namefont, fontsize), text=tirage[5])

            labels[6].place(x=x[6], y=y[0], anchor=CENTER)
            labels[6].configure(font=(namefont, fontsize), text=tirage[6])

            labels[7].place(x=x[7], y=y[0], anchor=CENTER)
            labels[7].configure(font=(namefont, fontsize), text=tirage[7])

            labels[8].place(x=x[8], y=y[0], anchor=CENTER)
            labels[8].configure(font=(namefont, fontsize), text=tirage[8])

            labels[9].place(x=x[9], y=y[0], anchor=CENTER)
            labels[9].configure(font=(namefont, fontsize), text=tirage[9])

            labels[10].place(x=x[0]+80, y=y[0]+130, anchor=CENTER)
            labels[10].configure(font=(namefont, int(0.6*fontsize)), text=etat.nbLettres+' L', fg='dimgray')

            labels[11].place(x=x[0]-100, y=y[0]-50, anchor=CENTER)
            labels[11].configure(font=(namefont, fontsize), text='')
        if len(tirage) == 11:
            labels[0].place(x=x[0]-17.5, y=y[0], anchor=CENTER)
            labels[0].configure(font=(namefont, fontsize), text=tirage[0])

            labels[1].place(x=x[1]-17.5, y=y[0], anchor=CENTER)
            labels[1].configure(font=(namefont, fontsize), text=tirage[1])

            labels[2].place(x=x[2]-17.5, y=y[0], anchor=CENTER)
            labels[2].configure(font=(namefont, fontsize), text=tirage[2])

            labels[3].place(x=x[3]-17.5, y=y[0], anchor=CENTER)
            labels[3].configure(font=(namefont, fontsize), text=tirage[3])

            labels[4].place(x=x[4]-17.5, y=y[0], anchor=CENTER)
            labels[4].configure(font=(namefont, fontsize), text=tirage[4])

            labels[5].place(x=x[4]+17.5, y=y[0], anchor=CENTER)
            labels[5].configure(font=(namefont, fontsize), text=tirage[5])

            labels[6].place(x=x[5]+17.5, y=y[0], anchor=CENTER)
            labels[6].configure(font=(namefont, fontsize), text=tirage[6])

            labels[7].place(x=x[6]+17.5, y=y[0], anchor=CENTER)
            labels[7].configure(font=(namefont, fontsize), text=tirage[7])

            labels[8].place(x=x[7]+17.5, y=y[0], anchor=CENTER)
            labels[8].configure(font=(namefont, fontsize), text=tirage[8])

            labels[9].place(x=x[8]+17.5, y=y[0], anchor=CENTER)
            labels[9].configure(font=(namefont, fontsize), text=tirage[9])

            labels[10].place(x=x[9]+17.5, y=y[0], anchor=CENTER)
            labels[10].configure(font=(namefont, fontsize), text=tirage[10])

            labels[11].place(x=x[0] +80, y=y[0]+130, anchor=CENTER)
            labels[11].configure(font=(namefont, int(0.6*fontsize)), text=etat.nbLettres+' L', fg='dimgray')
    else:
        cha = etat.solution
...
valider.place(x=340, y=200, anchor=CENTER, width=150, height=50)
...
oublis.place(x=530, y=200, anchor=CENTER, width=150, height=50)
...
label_nb.place(x=730, y=200, anchor=CENTER)
...
T = tk.Text(fenetre)
T.insert(INSERT, '')
T.place(x=40, y=250, width=750, height=95)
T.configure(font=("Helvetica", 14))

uuu = Canvas(fenetre, height=2, width=750)
#uuu.pack(side=LEFT)
uuu.place(x=40, y=150, anchor='w')
uuu.create_line(0, 1, 750, 1, width=4, fill='dimgray')
